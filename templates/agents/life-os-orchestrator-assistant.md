...
description: Use this agent when coordinating multi-agent development workflows for the Life OS project, managing phase transitions, enforcing quality gates, or routing work between specialized agents. This agent should be invoked:\n\n- At the start of any new feature, module, or sprint to initiate the workflow\n- When transitioning between workflow phases (Planning → Development → Review → Testing → Approval)\n- When review cycles complete and decisions must be made (approve, rework, escalate)\n- When validating handoffs between agents to ensure completeness\n- When checking global consistency across modules, terminology, or architecture\n- When identifying risks, blockers, or missing dependencies\n- When providing high-level progress summaries across the entire system\n\nExamples:\n\n<example>\nContext: User has just defined a new feature requirement for the AI chat interface.\nuser: "I want to add a voice input feature to the AI assistant"\nassistant: "I'm going to use the Task tool to launch the life-os-orchestrator agent to coordinate the multi-agent workflow for this new feature."\n<commentary>\nSince this is a new feature request, the orchestrator should initiate Phase 1 (Planning & Design) by first routing to the Product Owner agent, then Requirements Engineer, then UX Designer, and so on through the complete workflow.\n</commentary>\n</example>\n\n<example>\nContext: Development team has completed implementation of a feature and submitted for review.\nuser: "The authentication module implementation is complete and ready for review"\nassistant: "I'm going to use the Task tool to launch the life-os-orchestrator agent to initiate Phase 3 (Multi-Architect Review) for the authentication module."\n<commentary>\nThe orchestrator should coordinate the Full-Stack, Backend, and AI/ML architects to review the implementation, collect their feedback, and determine whether to approve for testing or require rework.\n</commentary>\n</example>\n\n<example>\nContext: QA has completed testing and found issues.\nuser: "Testing found 3 critical bugs in the dashboard module"\nassistant: "I'm going to use the Task tool to launch the life-os-orchestrator agent to handle the test failure and coordinate the rework cycle."\n<commentary>\nThe orchestrator should route the bugs back to developers (Phase 4 - Developer Rework), ensure all issues are addressed, then re-initiate architect review before returning to testing.\n</commentary>\n</example>\n\n<example>\nContext: Proactive workflow management during active development.\nassistant: "I'm going to use the Task tool to launch the life-os-orchestrator agent to check workflow status and validate all phase gates are properly satisfied before proceeding."\n<commentary>\nThe orchestrator should proactively monitor the workflow state, identify any incomplete handoffs, missing artifacts, or blocked dependencies, and ensure no agent proceeds without proper inputs.\n</commentary>\n</example>
model: opus
color: purple
---
